...
func init() {
	// tpl = template.Must(template.ParseGlob("webui/*"))
	// tplIndex = template.Must(template.ParseFiles("webui/index.html"))
	// tplHeader = template.Must(template.ParseFiles("webui/include-header.html"))
	// tplFooter = template.Must(template.ParseFiles("webui/include-footer.html"))
	// indexTemplate := template.New("index_template")
	// indexTemplate = template.Must(template.ParseFiles("webui/include-header.html", "webui/index.html", "webui/include-footer.html"))
...
func indexGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err := os.Stat(filepath.Join(".", "webui/styles", "main.css"))
	Info.Println("css file read error: ", err)

	templates, err := template.ParseFiles("webui/include-header.html", "webui/index.html", "webui/include-footer.html")
...
func startWebServer() {
	...
	r.PathPrefix("/styles/").Handler(http.StripPrefix("/styles/",
		http.FileServer(http.Dir("webui/styles/"))))

	r.PathPrefix("/images/").Handler(http.StripPrefix("/images/",
		http.FileServer(http.Dir("webui/images/"))))
